...
#import "DBZDelegateController.h"
#import "DBZIOConsumer.h"
#import "DBZIOConsumerPool.h"
#import "DBZHash.h"
#import "DBZURLUtilities.h"
#import "DBZError.h"
#import "NSURLRequest+DBZWebSocket.h"
#import "NSRunLoop+DBZWebSocket.h"
#import "DBZProxyConnect.h"
#import "DBZSecurityPolicy.h"
#import "DBZHTTPConnectMessage.h"
#import "DBZRandom.h"
#import "DBZLog.h"
#import "DBZMutex.h"
#import "DBZSIMDHelpers.h"
#import "NSURLRequest+DBZWebSocketPrivate.h"
#import "NSRunLoop+DBZWebSocketPrivate.h"
#import "DBZConstants.h"
...
{
    import_NSURLRequest_DBZWebSocket();
    import_NSRunLoop_DBZWebSocket();
}
...
static NSString *const DBZWebSocketAppendToSecKeyString = @"258EAFA5-E914-47DA-95CA-C5AB0DC85B11";

static inline int32_t validate_dispatch_data_partial_string(NSData *data);

static uint8_t const DBZWebSocketProtocolVersion = 13;

NSString *const DBZWebSocketErrorDomain = @"DBZWebSocketErrorDomain";
NSString *const SRHTTPResponseErrorKey = @"HTTPResponseStatusCode";

@interface DBZWebSocket ()  <NSStreamDelegate>
...
@property (nonatomic, strong, readonly) DBZDelegateController *delegateController;

@end

@implementation DBZWebSocket {
    DBZMutex _kvoLock;
    OSSpinLock _propertyLock;

    dispatch_queue_t _workQueue;
    NSMutableArray<DBZIOConsumer *> *_consumers;
    ...
    DBZSecurityPolicy *_securityPolicy;
    ...
    __strong DBZWebSocket *_selfRetain;

    NSArray<NSString *> *_requestedProtocols;
    DBZIOConsumerPool *_consumerPool;

    // proxy support
    DBZProxyConnect *_proxyConnect;
}
...
- (instancetype)initWithURLRequest:(NSURLRequest *)request protocols:(NSArray<NSString *> *)protocols securityPolicy:(DBZSecurityPolicy *)securityPolicy
{
    self = [super init];
    if (!self) return self;
    ...
    _consumerPool = [[DBZIOConsumerPool alloc] init];
...
{
    DBZSecurityPolicy *securityPolicy;
    BOOL certificateChainValidationEnabled = !allowsUntrustedSSLCertificates;

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated"

    securityPolicy = [[DBZSecurityPolicy alloc] initWithCertificateChainValidationEnabled:certificateChainValidationEnabled];
...
- (instancetype)initWithURLRequest:(NSURLRequest *)request securityPolicy:(DBZSecurityPolicy *)securityPolicy
{
...
- (instancetype)initWithURL:(NSURL *)url securityPolicy:(DBZSecurityPolicy *)securityPolicy
{
...
{
    assert(_url);
    NSAssert(self.readyState == SR_CONNECTING, @"Cannot call -(void)open on DBZWebSocket more than once.");
    ...
    if (_urlRequest.timeoutInterval > 0) {
        ...
        dispatch_after(popTime, dispatch_get_main_queue(), ^{
            if (self.readyState == SR_CONNECTING) {
                NSError *error = DBZErrorWithDomainCodeDescription(NSURLErrorDomain, NSURLErrorTimedOut, @"Timed out connecting to server.");
                [self _failWithError:error];
            }
        });
    }

    _proxyConnect = [[DBZProxyConnect alloc] initWithURL:_url];
...
- (void)_connectionDoneWithError:(NSError *)error readStream:(NSInputStream *)readStream writeStream:(NSOutputStream *)writeStream
{
    ...
    // Schedule to run on a work queue, to make sure we don't run this inline and deallocate `self` inside `DBZProxyConnect`.
...
- (BOOL)_checkHandshake:(CFHTTPMessageRef)httpMessage;
{
    ...
    NSString *concattedString = [_secKey stringByAppendingString:DBZWebSocketAppendToSecKeyString];
...
- (void)_HTTPHeadersDidFinish;
{
    ...
    if(![self _checkHandshake:_receivedHTTPHeaders]) {
        NSError *error = DBZErrorWithCodeDescription(2133, @"Invalid Sec-WebSocket-Accept response.");
        [self _failWithError:error];
        return;
    }
    ...
    if (negotiatedProtocol) {
        ...
        if ([_requestedProtocols indexOfObject:negotiatedProtocol] == NSNotFound) {
            NSError *error = DBZErrorWithCodeDescription(2133, @"Server specified Sec-WebSocket-Protocol that wasn't requested.");
            [self _failWithError:error];
            return;
        }
...
- (void)_readHTTPHeader;
{
    ...
    [self _readUntilHeaderCompleteWithCallback:^(DBZWebSocket *socket,  NSData *data) {
...
- (void)didConnect;
{
    ...
    CFHTTPMessageRef message = DBZHTTPConnectMessageCreate(_urlRequest,
                                                          _secKey,
                                                          DBZWebSocketProtocolVersion,
                                                          self.requestCookies,
                                                          _requestedProtocols);
...
- (void)_handleFrameHeader:(frame_header)frame_header curData:(NSData *)curData;
{
    ...
    if (frame_header.payload_length == 0) {
        ...
        [self _addConsumerWithDataLength:(size_t)frame_header.payload_length callback:^(DBZWebSocket *sself, NSData *newData) {
            if (isControlFrame) {
...
- (void)_readFrameContinue;
{
    ...
    [self _addConsumerWithDataLength:2 callback:^(DBZWebSocket *sself, NSData *data) {
        ...
        if (extra_bytes_needed == 0) {
            ...
            [sself _addConsumerWithDataLength:extra_bytes_needed callback:^(DBZWebSocket *eself, NSData *edata) {
...
- (void)_pumpWriting;
{
    ...
    if (dataLength - _outputBufferOffset > 0 && _outputStream.hasSpaceAvailable) {
        ...
        if (streamFailed) {
            ...
            NSError *error = streamError ? DBZErrorWithCodeDescriptionUnderlyingError(code, description, streamError) : DBZErrorWithCodeDescription(code, description);
            [self _failWithError:error];
            return;
        }
...
- (void)_scheduleCleanup
{
    @synchronized(self) {
        ...
        // This way we'll prevent race conditions between handleEvent and DBZWebSocket's dealloc
...
- (BOOL)_innerPumpScanner {
    ...
    DBZIOConsumer *consumer = [_consumers objectAtIndex:0];
...
- (void)stream:(NSStream *)aStream handleEvent:(NSStreamEvent)eventCode
{
    ...
    if (_requestRequiresSSL && !_streamSecurityValidated &&
        ...
        if (!_streamSecurityValidated) {
            dispatch_async(_workQueue, ^{
                NSError *error = DBZErrorWithDomainCodeDescription(NSURLErrorDomain,
        ...
        case NSStreamEventHasBytesAvailable: {
            ...
            while (_inputStream.hasBytesAvailable) {
                ...
                if (bytesRead > 0) {
                    ...
                    if (!data) {
                        NSError *error = DBZErrorWithCodeDescription(SRStatusCodeMessageTooBig,
...
- (id<DBZWebSocketDelegate> _Nullable)delegate
{
    return self.delegateController.delegate;
}

- (void)setDelegate:(id<DBZWebSocketDelegate> _Nullable)delegate
{
