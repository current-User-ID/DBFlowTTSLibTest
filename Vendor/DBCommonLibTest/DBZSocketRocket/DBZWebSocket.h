...
@class DBZWebSocket;
@class DBZSecurityPolicy;

/**
 Error domain used for errors reported by DBZWebSocket.
 */
extern NSString *const DBZWebSocketErrorDomain;
...
@protocol DBZWebSocketDelegate;

///--------------------------------------
#pragma mark - DBZWebSocket
///--------------------------------------

/**
 A `DBZWebSocket` object lets you connect, send and receive data to a remote Web Socket.
 */
@interface DBZWebSocket : NSObject <NSStreamDelegate>
 ...
 */
@property (nonatomic, weak) id <DBZWebSocketDelegate> delegate;
...
/**
 ...
 @param securityPolicy Policy object describing transport security behavior.
 */
- (instancetype)initWithURLRequest:(NSURLRequest *)request protocols:(nullable NSArray<NSString *> *)protocols securityPolicy:(DBZSecurityPolicy *)securityPolicy NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 An instance of `DBZWebSocket` is intended for one-time-use only. This method should be called once and only once.
 */
- (void)open;
...
/**
 The `DBZWebSocketDelegate` protocol describes the methods that `DBZWebSocket` objects
 call on their delegates to handle status and messsage events.
 */
@protocol DBZWebSocketDelegate <NSObject>
...
/**
 ...
 @param webSocket An instance of `DBZWebSocket` that received a message.
 ...
 */
- (void)webSocket:(DBZWebSocket *)webSocket didReceiveMessageWithString:(NSString *)string;
 ...
 */
- (void)webSocket:(DBZWebSocket *)webSocket didReceiveMessageWithData:(NSData *)data;
...
/**
 ...
 @param webSocket An instance of `DBZWebSocket` that was open.
 */
...
/**
 ...
 @param webSocket An instance of `DBZWebSocket` that failed with an error.
...
/**
 ...
 @param webSocket An instance of `DBZWebSocket` that was closed.
 ...
 */
- (void)webSocket:(DBZWebSocket *)webSocket didCloseWithCode:(NSInteger)code reason:(nullable NSString *)reason wasClean:(BOOL)wasClean;

/**
 Called on receive of a ping message from the server.

 @param webSocket An instance of `DBZWebSocket` that received a ping frame.
 @param data      Payload that was received or `nil` if there was no payload.
 */
- (void)webSocket:(DBZWebSocket *)webSocket didReceivePingWithData:(nullable NSData *)data;

/**
 Called when a pong data was received in response to ping.

 @param webSocket An instance of `DBZWebSocket` that received a pong frame.
 @param pongData  Payload that was received or `nil` if there was no payload.
 */
- (void)webSocket:(DBZWebSocket *)webSocket didReceivePong:(nullable NSData *)pongData;
...
/**
 ...
 @param webSocket An instance of `DBZWebSocket` that received a text frame.

 @return `YES` if text frame should be converted to UTF-8 String, otherwise - `NO`. Default: `YES`.
 */
- (BOOL)webSocketShouldConvertTextFrameToString:(DBZWebSocket *)webSocket NS_SWIFT_NAME(webSocketShouldConvertTextFrameToString(_:));
